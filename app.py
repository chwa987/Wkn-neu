...
st.set_page_config(page_title="Momentum-RoboAdvisor v1.2.1", page_icon="📈", layout="wide")
...
# ---------------------------- #
# Marktfilter (Dual Momentum Light)
# ---------------------------- #

def apply_market_filter(bm_prices):
    if bm_prices.empty:
        return True
...
def run_backtest_weekly(prices, volumes, benchmark, start_date, end_date,
                        top_n=10, min_volume=5_000_000, max_dd52=-30,
                        max_volatility=1.0, apply_benchmark=True,
    ...
    for i in range(len(rebal_days)-1):
        ...
        bm_slice = pd.DataFrame({"BM": benchmark.loc[:asof]}) if benchmark is not None else None

        snap = compute_indicators(p_slice, v_slice, benchmark_df=bm_slice)
        if snap.empty or not apply_market_filter(bm_slice):
...
st.title("📊 Momentum-Analyse v1.2.1 (mit Marktfilter + Backtest)")
